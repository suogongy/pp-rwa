...
contract RWA20 is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    function mint(address to, uint256 amount) public onlyOwner whenNotPaused nonReentrant {
        ...
        bytes32 txId = keccak256(abi.encodePacked(block.timestamp, to, amount, blockhash(block.number - 1), msg.sender));
    ...
    function burn(uint256 amount) public whenNotPaused nonReentrant {
        ...
        bytes32 txId = keccak256(abi.encodePacked(block.timestamp, msg.sender, amount, blockhash(block.number - 1)));
        ...
        nonReentrant
    {
        ...
        bytes32 batchId =
            keccak256(abi.encodePacked(block.timestamp, sender, recipients.length, blockhash(block.number - 1)));
     ...
     */
    function emergencyExtract(address tokenAddress, uint256 amount) public onlyOwner whenPaused {
    ...
    {
        return (name(), symbol(), decimals(), owner(), paused(), totalSupply(), version());
    }
